...
function solution(hp) {
  // hp에서 장군 개미의 공격력 만큼 나누어 장군개미 수를 구한다.
  const generalAnt = Math.floor(hp / 5)
  // hp에서 장군 개미의 공격력을 나누고 나머지에서 병정 개미의 공격력 만큼 나누어 병정개미 수를 구한다.
  const soldierAnt = Math.floor(hp % 5 / 3)
  // hp에서 장군 개미와 병정 개미의 공격력을 나누고 나머지는 일개미 수이다.
  const workAnt = Math.floor(hp % 5 % 3)
  // 모든 개미의 수를 더하여 반환한다.
